	...
	while(1)
	{
		...
		else if(eh->ether_type == htons(ETH_P_IP))
		{
			...
			{	
				...
				printf("Type = %hi\n", eh->ether_type);
				printf("Payload = ");
				for(int i = sizeof(struct ether_header) + sizeof(struct iphdr) + sizeof(struct udphdr); i < recvLen; i++)
				{
					printf("i=%d", i);
					printf("%c", buf[i]);
				}
...
void send_img(char* iName, unsigned char* destIPRaw, char* imageFile)
{
        printf("Inside Send\n");
	int sockfd, i, byteSent, tx_length = 0, header_length = 0;
	struct ifreq if_mac;
	struct ifreq if_idx;
	struct sockaddr_ll sk_addr;
	char sendbuf[BUF_SIZ];
	...
	eh->ether_dhost[5] = address[5];
	eh->ether_type = htons(ETH_P_IP);
	header_length += sizeof(struct ether_header);
	...
	FILE *fp;
        ...
        while(fgets(buf, 80, fp) != NULL){
	        ...
	        ip->tot_len = htons(sizeof(struct iphdr) + sizeof(struct udphdr) + strlen(buf));
	        ip->check = ip_checksum(ip, 20);
	        udph->len = htons(strlen(buf));
	        udph->check = ip_checksum(udph, 8 + strlen(buf));
	        for(tx_length = 0; tx_length < strlen(buf); tx_length++)
		{
			sendbuf[header_length+i] = buf[i];
		}
	        while(!ack)
	        {
	                byteSent = sendto(sockfd, sendbuf, header_length + tx_length, 0,
		                (struct sockaddr*)&sk_addr, sk_addr_size);
		        ack = 1;
		}        
...
int main(int argc, char *argv[])
{
	...
	if (argc > 1){
		...
		else if(strncmp(argv[1],"S_IMG", 4)==0)
		{
			if (argc == 5)
			{
				mode = S_IMG;
	...
	{
		send_img(interfaceName, dest_ip, img);
	}
	return 0;
}
